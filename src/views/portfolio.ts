...
function renderHoldingsTable(pd: PortfolioData, snaps: Snapshot[]): void {
  ...
  document.getElementById('port-table').innerHTML = `
    ...
    <div class="tbl-row th hold-row" role="row" id="port-table-header">
      ...
      <div role="columnheader" class="sortable-th${_holdSort.key === 'cost' ? ' sort-active' : ''}" data-sort-key="cost" aria-sort="${_holdSort.key === 'cost' ? (_holdSort.dir === 'asc' ? 'ascending' : 'descending') : 'none'}" style="text-align:right"><span class="th-label">Cost basis${infoTip('Total amount invested (net of sells). Calculated from your imported CSV transactions using the method chosen in Settings.')}</span><span class="sort-arrow">${_holdSort.key === 'cost' ? (_holdSort.dir === 'asc' ? ' \u25b2' : ' \u25bc') : ''}</span></div>
      ...
      <div role="columnheader" class="sortable-th${_holdSort.key === 'realizedPnL' ? ' sort-active' : ''}" data-sort-key="realizedPnL" aria-sort="${_holdSort.key === 'realizedPnL' ? (_holdSort.dir === 'asc' ? 'ascending' : 'descending') : 'none'}" style="text-align:right"><span class="th-label">Realized P&amp;L${infoTip('Gain or loss already locked in from shares you have sold (proceeds minus their cost basis, fees included). Separate from unrealized gain on shares still held. Changes if you switch the cost-basis method in Settings.')}</span><span class="sort-arrow">${_holdSort.key === 'realizedPnL' ? (_holdSort.dir === 'asc' ? ' \u25b2' : ' \u25bc') : ''}</span></div>
