...
describe('fetchRate', () => {
  ...
  it('returns a correctly shaped FxRateRecord', async () => {
    ...
    mockFetch({ base: 'USD', date: '2024-01-15', rates: { EUR: 0.92 } });
  ...
  it('sets date to effectiveDate for "latest" requests', async () => {
    mockFetch({ base: 'USD', date: '2024-01-19', rates: { EUR: 0.91 } });
  ...
  it('throws FrankfurterError when rates field is missing', async () => {
    mockFetch({ base: 'USD', date: '2024-01-15' }); // no "rates"
    await expect(fetchRate('USD', 'EUR', '2024-01-15')).rejects.toThrow(FrankfurterError);
  });

  it('throws FrankfurterError when target currency is absent from rates', async () => {
    mockFetch({ base: 'USD', date: '2024-01-15', rates: { GBP: 0.79 } });
    await expect(fetchRate('USD', 'EUR', '2024-01-15')).rejects.toThrow(FrankfurterError);
  });

  it('throws FrankfurterError when rate value is non-positive', async () => {
    mockFetch({ base: 'USD', date: '2024-01-15', rates: { EUR: 0 } });
    await expect(fetchRate('USD', 'EUR', '2024-01-15')).rejects.toThrow(FrankfurterError);
  });
