...
export async function loadHoldings(): Promise<Holding[]> {
  const db = await getDb();
  const result = db.exec(
    'SELECT isin, name, short_name, color, acc, active, target_pct, contrib_amount, contrib_interval, asset_class, region, fold_into, "order", ter FROM holdings ORDER BY "order" ASC',
  );
  if (result.length === 0) return [];
...
export async function saveHoldings(holdings: Holding[]): Promise<void> {
  const db = await getDb();
  const stmt = db.prepare(
    'INSERT INTO holdings (isin, name, short_name, color, acc, active, target_pct, contrib_amount, contrib_interval, asset_class, region, fold_into, "order", ter) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  );
  try {
    db.run('BEGIN');
...
function rowToHolding(row: unknown[]): Holding {
  ...
  return {
    ...
    contribAmount: Number(row[7]) || 0,
    contribInterval: (String(row[8] ?? 'weekly') as ContribInterval) || 'weekly',
    assetClass: String(row[9] ?? ''),
    region: String(row[10] ?? ''),
    foldInto: String(row[11] ?? ''),
